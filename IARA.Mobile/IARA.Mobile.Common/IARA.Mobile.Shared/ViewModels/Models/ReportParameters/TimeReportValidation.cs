...
{
    public class TimeReportValidation : TLBaseViewModel, IReportValidation
    {
        public TimeReportValidation(bool isMandatory, string defaultValue, string pattern, string errorMessage)
        {
            this.AddValidation();
            ...
            if (TimeSpan.TryParse(defaultValue, out TimeSpan result))
            {
                ValidState.Value = result;
            }
        }

        public ValidStateTime ValidState { get; set; }

        IValidState IReportValidation.ValidState => ValidState;

        public string GetValue()
        {
            return ValidState.Value?.ToString();
        }
    }
}
