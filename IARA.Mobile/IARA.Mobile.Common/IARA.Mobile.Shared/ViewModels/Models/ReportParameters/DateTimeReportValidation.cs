...
{
    public class DateTimeReportValidation : TLBaseViewModel, IReportValidation
    {
        public DateTimeReportValidation(bool isMandatory, string defaultValue, string pattern, string errorMessage)
        {
            this.AddValidation();
        ...
        public ValidStateDateTime ValidState { get; set; }

        IValidState IReportValidation.ValidState => ValidState;

        public string GetValue()
        {
            return ValidState.Value?.ToString("yyyy-MM-dd HH:mm:ss");
        }
    }
}
