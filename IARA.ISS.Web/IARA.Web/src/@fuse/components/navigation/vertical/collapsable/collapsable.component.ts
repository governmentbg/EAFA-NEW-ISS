...
export class FuseNavVerticalCollapsableComponent implements OnInit, OnDestroy {
     ...
     *
     * @param parent: FuseNavigationItem
     * @param item: FuseNavigationItem
     * @returns {boolean}
     */
    isChildrenOf(parent?: FuseNavigationItem, item?: FuseNavigationItem): boolean {
        ...
        if (item !== undefined && children.indexOf(item as FuseNavigationItem) > -1) {
            return true;
        }
     ...
     * @returns {boolean}
     */
    isUrlInChildren(parent?: FuseNavigationItem, url?: string): boolean {
        const children = parent?.children;
        ...
        for (const child of children) {
            ...
            if ((child.url !== undefined && child.url as string === url) /*|| (child.url !== undefined && url?.includes(child.url as string))*/) {
                return true;
            }
        }
