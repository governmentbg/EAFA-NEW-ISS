...
{
    public class VesselToFluxVesselReportMapper : BaseService, IVesselToFluxVesselReportMapper
    {
        ...
        public VesselToFluxVesselReportMapper(IARADbContext dbContext,
        ...
        private FLUXReportVesselInformationType CreateReportVesselInformation(List<ShipRegisterEditDTO> vessels, string reportType, ReportPurposeCodes purpose)
        {
            ...
            ValidateFLUXReportVesselInformationType(result);
        ...
        private void ValidateFLUXReportVesselInformationType(FLUXReportVesselInformationType report)
        {
            if (report == null)
            {
                throw new ArgumentException("No FLUXReportVesselInformationType provided");
            }

            ValidateFLUXReportDocument(report.FLUXReportDocument);
            ValidateVesselEvents(report.VesselEvent);
        }

        private void ValidateFLUXReportDocument(FLUXReportDocumentType document)
        {
            if (document == null)
            {
                throw new ArgumentException("No FLUXReportDocumentType provided");
            }

            // Identification
            if (document.ID == null || document.ID.Length == 0)
            {
                throw new ArgumentException("The report identification is not provided");
            }

            if (document.ID.Any(x => x.schemeID != IDTypes.UUID || string.IsNullOrEmpty(x.Value)))
            {
                throw new ArgumentException("The report identification must be UUID and not null");
            }

            // Report type
            if (document.TypeCode == null || string.IsNullOrEmpty(document.TypeCode.Value))
            {
                throw new ArgumentException("The type of report is not provided");
            }

            if (document.TypeCode.listID != ListIDTypes.FLUX_VESSEL_REPORT_TYPE)
            {
                throw new ArgumentException("The list referenced in the Report Type is not a valid");
            }

            if (!Db.MdrFluxVesselReportTypes.Any(x => x.Code == document.TypeCode.Value))
            {
                throw new ArgumentException("The Report Type is not a valid code");
            }

            // Creation
            if (document.CreationDateTime == null)
            {
                throw new ArgumentException("The date of the report is not provided");
            }

            if (document.CreationDateTime.Item > DateTime.Now)
            {
                throw new ArgumentException("The creation date of the report should not be in the future");
            }

            // Purpose
            if (document.PurposeCode == null || string.IsNullOrEmpty(document.PurposeCode.Value))
            {
                throw new ArgumentException("The purpose code is not provided");
            }

            if (document.PurposeCode.listID != ListIDTypes.FLUX_GP_PURPOSE)
            {
                throw new ArgumentException("The list referenced in Purpose is not valid");
            }

            if (!Db.MdrFluxGpPurposes.Any(x => x.Code == document.PurposeCode.Value))
            {
                throw new ArgumentException("The purpose code is not valid");
            }

            // Party
            if (document.OwnerFLUXParty == null || document.OwnerFLUXParty.ID == null || document.OwnerFLUXParty.ID.Length != 1)
            {
                throw new ArgumentException("The sender identification of the report is not provided");
            }

            if (document.OwnerFLUXParty.ID[0].schemeID != ListIDTypes.FLUX_GP_PARTY)
            {
                throw new ArgumentException("The list referenced for the Party identifier is not valid");
            }

            if (!Db.MdrFluxGpParties.Any(x => x.Code == document.OwnerFLUXParty.ID[0].Value))
            {
                throw new ArgumentException("The sender identification is not a valid code");
            }
        }

        private void ValidateVesselEvents(VesselEventType[] events)
        {
            if (events == null || events.Length == 0)
            {
                throw new ArgumentException("At least one vessel event must be provided");
            }

            foreach (VesselEventType vesselEvent in events)
            {
                ValidateVesselEvent(vesselEvent);
            }
        }

        private void ValidateVesselEvent(VesselEventType vesselEvent)
        {
            if (vesselEvent == null)
            {
                throw new ArgumentException("Vessel event must not be null");
            }

            ValidateEventTypeAndOccurence(vesselEvent);
            ValidateVesselTransportMeans(vesselEvent.RelatedVesselTransportMeans);
        }

        private void ValidateEventTypeAndOccurence(VesselEventType vesselEvent)
        {
            if (vesselEvent.TypeCode == null || string.IsNullOrEmpty(vesselEvent.TypeCode.Value))
            {
                throw new ArgumentException("Vessel event type must be provided");
            }

            if (vesselEvent.TypeCode.listID != ListIDTypes.VESSEL_EVENT)
            {
                throw new ArgumentException("Vessel event type listID must be VESSEL_EVENT");
            }

            if (!Db.MdrVesselEvents.Any(x => x.Code == vesselEvent.TypeCode.Value))
            {
                throw new ArgumentException("Vessel event must have valid code");
            }

            if (vesselEvent.OccurrenceDateTime == null)
            {
                throw new ArgumentException("The date of event must be provided");
            }
        }

        private void ValidateVesselTransportMeans(VesselTransportMeansType vessel)
        {
