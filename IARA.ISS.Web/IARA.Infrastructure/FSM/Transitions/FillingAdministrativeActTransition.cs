...
{
    [Connection(TransitionCode = Enums.TransitionCodesEnum.FillAdmAct)]
    internal class FillingAdministrativeActTransition : Transition
    {
        protected override string ActionErrorMessage => "Use the overload with id and statusReason as parameters.";

        public FillingAdministrativeActTransition(TransitionContext context)
            : base(context)
        { }

        public override bool CanTransition(int id, bool isTriggeredManually, string statusReason)
        {
            return this.HasRegisterEntry(id, RecordTypesEnum.Register);
        }
