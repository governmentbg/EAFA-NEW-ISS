...
{
    [Connection(TransitionCode = Enums.TransitionCodesEnum.FileInMustPayService)]
    internal class FileInMustPayServiceTransition : Transition
    {
        protected override string ActionErrorMessage => "Use the overload with id, eventisData and statusReason as parameters.";

        public FileInMustPayServiceTransition(TransitionContext context)
        : base(context)
        { }
        ...
        public override bool CanTransition(int id, EventisDataDTO eventisData)
        {
            ...
            return paymentStatus == PaymentStatusesEnum.Unpaid && eventisData != null && !string.IsNullOrEmpty(eventisData.EventisNumber);
        }
