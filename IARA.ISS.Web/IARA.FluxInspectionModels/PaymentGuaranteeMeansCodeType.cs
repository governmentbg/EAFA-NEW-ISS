namespace IARA.FluxInspectionModels
{
    [Serializable]
    [XmlType(Namespace = "urn:un:unece:uncefact:data:standard:QualifiedDataType:32")]
    public partial class PaymentGuaranteeMeansCodeType
    {

        private string listIDField;

        private PaymentGuaranteeMeansCodeListAgencyIDContentType listAgencyIDField;

        private bool listAgencyIDFieldSpecified;

        private string listVersionIDField;

        private PaymentGuaranteeMeansCodeContentType valueField;

        public PaymentGuaranteeMeansCodeType()
        {
            this.listIDField = "4431";
            this.listAgencyIDField = PaymentGuaranteeMeansCodeListAgencyIDContentType.Item6;
            this.listVersionIDField = "D22A";
        }

        [XmlAttribute(DataType = "token")]
        public string listID
        {
            get => this.listIDField;
            set => this.listIDField = value;
        }

        [XmlAttribute]
        public PaymentGuaranteeMeansCodeListAgencyIDContentType listAgencyID
        {
            get => this.listAgencyIDField;
            set => this.listAgencyIDField = value;
        }

        [XmlIgnore()]
        public bool listAgencyIDSpecified
        {
            get => this.listAgencyIDFieldSpecified;
            set => this.listAgencyIDFieldSpecified = value;
        }

        [XmlAttribute(DataType = "token")]
        public string listVersionID
        {
            get => this.listVersionIDField;
            set => this.listVersionIDField = value;
        }

        [XmlText]
        public PaymentGuaranteeMeansCodeContentType Value
        {
            get => this.valueField;
            set => this.valueField = value;
        }
    }
}